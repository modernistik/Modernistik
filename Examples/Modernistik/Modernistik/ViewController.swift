...
class ViewController: UITableViewController {
    ...
    override func tableView(_: UITableView, numberOfRowsInSection section: Int) -> Int {
        let dataSource = data(for: section)
        return dataSource.count
    }

    override func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        let dataSource = data(for: indexPath.section)
        return dataSource.height(forRow: indexPath.row)
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // select the right data source
        let dataSource = data(for: indexPath.section)
        return dataSource.cell(forRow: indexPath.row, in: tableView)
    }

    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let dataSource = data(for: indexPath.section)
        // forward the call
        dataSource.tableView(tableView, didSelectRow: indexPath.row)
    }
}
